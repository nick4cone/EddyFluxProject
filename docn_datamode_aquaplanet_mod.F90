module docn_datamode_aquaplanet_mod
  ...
  real(r8) , parameter :: perturb_max= 2._r8
  real(r8) , parameter :: wavenumber = 1._r8
  ...
  subroutine docn_datamode_aquaplanet_advance(exportState, model_mesh, sst_option, sst_constant_value, rc)
    ...
    if (present(sst_constant_value)) then
       ...
       if (sst_option < 1 .or. sst_option > 11) then
          call shr_sys_abort ('docn_prescribed_sst: ERROR: sst_option must be between 1 and 11')
       end if
       ...
       if (sst_option == 4) then ! Peaked
          do i = 1,lsize
             if (abs(rlat(i)) > maxlat) then
                ...
                ! Check if current latitude is within latrad8 of latcen
                if (abs(rlat(i)-latcen) <= latrad8) then
                   tmp1 = perturb_max*cos(rlon(i)*wavenumber)*(cos(pi*(rlat(i)-latcen)/(2._r8*latrad8))**2)
                   So_t(i) = So_t(i) + tmp1
                end if
             end if
          end do
